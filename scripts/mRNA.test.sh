...
>for id in "${samples[@]}"; do
     ...
     >python ~/2-O-Me/NanoNm/extract_raw_and_feature_fast_AUCG.py --cpu=30 --fl="${id}_guppy.list" -o "$id" --clip=5
     >python ~/2-O-Me/NanoNm/predict_sites_Nm.final.py --model ./model -i "$id" -o "${id}_Nm_model" -r  gencode.v27.transcripts.fa -g GRCh38.p13.genome.fa -b hg38.gene2transcripts1.txt
